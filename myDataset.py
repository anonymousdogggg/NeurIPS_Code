...
class ColumnarDataset(Dataset):
    def __init__(self, features, y, sex_feat, device):
        ...
        self.group_label = torch.tensor(self.group_label).to(device)
    ...
    def shuffle(self):
        idx = np.random.permutation(len(self.label_y))
        self.data = self.data[idx]
        self.label_y = self.label_y[idx]
        self.group_label = self.group_label[idx]
        self.group1_idx = torch.where(self.group_label==0)[0]
        self.group2_idx = torch.where(self.group_label==1)[0]
        self.y = self.label_y

# class ColumnarDataset1(Dataset):
#     def __init__(self, features, y, sex_feat, device):
#         num_data = sex_feat.shape[0]
#         self.group2_idx_set = set(list(sex_feat.nonzero()[0]))
#         self.group1_idx_set = set(list(range(num_data))) - self.group2_idx_set
...
#
#     def __getitem__(self, idx):
#         group_label = torch.tensor(self.group_label[idx], dtype=torch.long).to(self.device)
#         return self.idxes[idx], self.features[idx], self.y[idx], self.label_y[idx], group_label
...
class CelebaDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.pretrain_data is not None:
            return self.idxs[index], self.pretrain_data[index], self.y[index], self.label_y[index]
        else:
            img = Image.open(os.path.join(self.img_dir, self.img_names[index]))
            if self.transform is not None:
                img = self.transform(img)
            return self.idxs[index], img, self.y[index], self.label_y[index]
