...
def dataloader_wrapper(trn_ds, val_ds, test_ds, all_ds=None, train_batch_size=1, valid_batch_size=1, test_batch_size=1,
    ...
    # return trn_ds, val_ds, test_ds, train_dataloader, valid_dataloader, test_dataloader
...
def load_data(args):
    if args.dataset == 'adults':
        ...
        trn_ds, val_ds, test_ds = compas_dataloader(args)
        args.raw_dim = trn_ds.features.shape[1]
    ...
    #     trn_ds, val_ds, test_ds, train_dataloader, valid_dataloader, test_dataloader \
    #         = dataloader_wrapper(trn_ds, val_ds, test_ds, num_train, num_val, num_test)
    # else:
    #     trn_ds, val_ds, test_ds, train_dataloader, valid_dataloader, test_dataloader \
    #         = dataloader_wrapper(trn_ds, val_ds, test_ds, args.train_batch_size,
    #                             args.valid_batch_size, args.test_batch_size)
    if args.optim == 'GD':
        num_train = len(trn_ds)
        num_val = len(val_ds)
        num_test = len(test_ds)
