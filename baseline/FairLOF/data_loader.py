...
def compas_dataloader(args):
    if not os.path.exists('../datasets/compas.pkl'):
        ...
        idx = list(group_idx1[:int(0.2 * len(group_idx1))]) + list(group_idx0)
        x_data = x_data[idx]
        YL = YL[idx]
        x_control = x_control[idx]
        # a = np.where(x_control == 0)[0]
        # b = np.where(x_control == 1)[0]
        ...
        data = {'data': x_data, 'label': YL, 'group': x_control, 'idx_train':idx_train, 'idx_test': idx_test, 'idx_val': idx_val}
        with open('../datasets/compas.pkl', 'wb') as f:
            pickle.dump(data, f)
    else:
